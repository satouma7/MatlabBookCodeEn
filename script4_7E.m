Tmax=10000;Xmax=100;            %Or, Tmax=5000;Xmax=20;
A=zeros(Xmax,Xmax,Tmax);I=zeros(Xmax,Xmax,Tmax);
A(:,:,1)=rand(Xmax,Xmax);  %Or, A(46:55,26:35,1)=1; A(76:80,66:70,1)=1; A(26:30,86:90,1)=1;
Ap=zeros(Xmax,Xmax);Ip=zeros(Xmax,Xmax);
dt=0.1;dx=1;dx2=dx*dx;
da=0.02;di=0.5;ka=0.03;ki=0.06;
c1=0.08;c2=-0.08;c3=0.1;c4=0.11;c5=0;c6=-0.15;  %Or, c3=0.01 or c3=0.2;
Apmax=0.2;Ipmax=0.5;
Atemp=A(:,:,1);Itemp=I(:,:,1);
...
for T=1:Tmax-1   
    Ap=c1*A(:,:,T)+c2*I(:,:,T)+c3;
    Ip=c4*A(:,:,T)+c5*I(:,:,T)+c6; 
    Ap=((Ap<Apmax)&(Ap>0)).*Ap+(Ap>Apmax).*Apmax;%Setting the upper and lower limits of Ap
    Ip=((Ip<Ipmax)&(Ip>0)).*Ip+(Ip>Ipmax).*Ipmax;%Setting the upper and lower limits of I
    Atemp=A(:,:,T);Itemp=I(:,:,T);               %Calculating the diffusion of A and I
