...
Etemp=zeros(Xmax,Xmax);Eright=Etemp;Eleft=Etemp;Eup=Etemp;Edown=Etemp;Emutant=ones(Xmax,Xmax);Emutant(6:20,6:20)=0;%Definition of the EGF mutant region
for T=1:Tmax-1
    Etemp=E(:,:,T);
    ...
    E(:,:,T+1)=dt*(de*(Eright+Eleft+Eup+Edown-4*Etemp)/dx2-ke*E(:,:,T)+ae*A(:,:,T).*(1-A(:,:,T)).*Emutant)+E(:,:,T);%Reaction diffusion of E in the presence of the EGF mutant region   
    for X=2: Xmax-1
        for Y=2:Xmax-1
